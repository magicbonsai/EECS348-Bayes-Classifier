...
def check_imports(source_name):
    ...
    with open(path + 'student_code.py',"r") as f:
        tokens = f.read().replace("\n", " ").split()
...
def main():
    ...
    bayes.train(path + 'train.txt')
    predict = bayes.classify(path + 'classifyA.txt')
    fA = f_score(path + 'answersA.txt',predict)
